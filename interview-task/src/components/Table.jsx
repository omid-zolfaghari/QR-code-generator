"use client"
import QrCode from "qrcode"
import { useState } from 'react';
import Table from '@mui/material/Table';
import TableBody from '@mui/material/TableBody';
import TableCell from '@mui/material/TableCell';
import TableContainer from '@mui/material/TableContainer';
import TableRow from '@mui/material/TableRow';
import Paper from '@mui/material/Paper';
import { Button, Container, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, TextField } from '@mui/material';
import { deleteUrl, updateUrl } from '@/app/utils/urlActions';
import Image from 'next/image';


export default function BasicTable({urls}) {

    const [open, setOpen] = useState(false);
    const [value, setValue] = useState("");
    const [src, setSrc]= useState("data:image/png;base64,")

    const handleClickOpen = (item) => {
      setOpen(true);
      setValue(item)
    };
  
    const handleClose = () => {
      setOpen(false);
    };

    const generateQrCode = async(URL)=>{
        const link =await QrCode.toDataURL(URL)
        console.log(link);
        setSrc(link)
    }


  return (
    <Container sx={{display : "flex", alignItems : "center", gap:"20px"}}>
    <TableContainer sx={{width :"800px"}} component={Paper}>
      <Table sx={{ minWidth: 650, backgroundColor : "#DEE5D4" }} aria-label="simple table">
        <TableBody>
          {urls.map((url) => (
            <TableRow
              key={url._id}
              sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
            >
              <TableCell component="th" scope="row">
                {url.url}
              </TableCell>
              <TableCell align="right"><Button onClick={async()=> await deleteUrl(`http://localhost:3000/api/url/${url._id}`)} color='error' variant='contained'>Delete</Button></TableCell>
              <TableCell onClick={()=>handleClickOpen(url)} align="right"><Button color='warning' variant='contained'>Edit</Button></TableCell>
              <TableCell onClick={()=>generateQrCode(url.url)} align="right"><Button color='success' variant='contained'>Show</Button></TableCell>
            </TableRow>
          ))}
          
        <Dialog
        open={open}
        onClose={handleClose}
        PaperProps={{
          component: 'form',
          onSubmit: (event) => {
            event.preventDefault();
            handleClose();
          },
        }}
      >
        <DialogTitle>Edit</DialogTitle>
        <DialogContent>
          <DialogContentText>
            To edit this URL, please enter your new URL here. We
            will send updates occasionally.
          </DialogContentText>
          <TextField
           onChange={(e)=> updateUrl(`http://localhost:3000/api/url/${value._id}`, e.target.value)}
            defaultValue={value.url}
            autoFocus
            required
            margin="dense"
            id="name"
            name="URL"
            label="new URL"
            type='text'
            fullWidth
            variant="standard"
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose}>Cancel</Button>
          <Button type="submit">Submit</Button>
        </DialogActions>
      </Dialog>
        </TableBody>
      </Table>
    </TableContainer>
    <Image priority src={src} height={300} alt="QR code" width={300} />
    </Container>
  );
}

